...
class AuthApiTest : FunSpec({
    test("testAuthSession") {
        sessionTest {
            ...
            ktor.client.post(Auth.Token(token = authToken)).shouldHaveStatus(HttpStatusCode.Found)
    ...
    test("testAuthWebSocket") {
        sessionTest {
            ktor.client.get(Events.SessionEvents(namespace = "test", sessionId = "test")).shouldHaveStatus(HttpStatusCode.Unauthorized)
            ktor.client.post(Auth.Token(token = authToken)).shouldHaveStatus(HttpStatusCode.Found)
            ktor.client.get(Events.SessionEvents(namespace = "test", sessionId = "test")).shouldHaveStatus(HttpStatusCode.NotFound)
        }
    }
})